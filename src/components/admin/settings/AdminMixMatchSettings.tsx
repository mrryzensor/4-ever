...
const TYPOGRAPHY_SCALE_CONTROLS = [
  { key: 'typographyTitleScale', label: 'Títulos principales', help: 'Nombres y títulos grandes', cssVar: '--invitation-type-title' },
...
const TypographyScaleControls: React.FC<{
  ...
  <div className="rounded-xl border border-stone-200 bg-stone-50/70 p-3 sm:p-4">
    <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
      ...
      <button
        type="button"
        onClick={() => onChange({ typographyTitleScale: 100, typographyHeadingScale: 100, typographyBodyScale: 100, typographySubtitleScale: 100, typographyDetailScale: 100, typographyBadgeScale: 100, typographyButtonScale: 100 })}
        ...
        return (
          <label key={key} className="rounded-lg border border-stone-200 bg-white p-3">
            <span className="flex items-center justify-between gap-2 text-xs font-semibold text-stone-800">
              <span>{label}</span><span className="tabular-nums text-stone-600">{value}%</span>
            </span>
            <span className="mt-0.5 block text-[11px] text-stone-500">{help}</span>
            <span
              className={key === 'typographyButtonScale'
            ...
            <input
              ...
              className={`mt-2 w-full ${accent === 'indigo' ? 'accent-indigo-700' : 'accent-purple-700'}`}
              aria-label={`Tamaño de ${label.toLowerCase()}`}
            />
          </label>
        );
      })}
