...
export const PhotoGallery: React.FC<PhotoGalleryProps> = ({
  ...
  const handlePrevPhoto = useCallback((e?: React.MouseEvent) => {
    ...
    void openPhotoAtIndex(currentIndex === 0 ? photos.length - 1 : currentIndex - 1);
  }, [activePhotoIndex, photos.length, openPhotoAtIndex]);

  const handleNextPhoto = useCallback((e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    if (photos.length === 0) return;
    const currentIndex = activePhotoIndex ?? 0;
    void openPhotoAtIndex(currentIndex === photos.length - 1 ? 0 : currentIndex + 1);
  }, [activePhotoIndex, photos.length, openPhotoAtIndex]);
  ...
  const navigateToCarouselPhoto = useCallback(async (nextIndex: number) => {
    ...
    setIsLoadingNextPhoto(false);
  }, [photos, loadedCarouselPhotoKeys]);

  // Preload and decode the first photo before guests reach this section.
  useEffect(() => {
  ...
  useEffect(() => {
    if (!isAutoPlay || isHovered || photos.length <= 1 || activePhotoIndex !== null || isLoadingNextPhoto || carouselLoadError || !isCurrentCarouselPhotoLoaded) return;
    const timer = setTimeout(() => {
      void navigateToCarouselPhoto(carouselIndex === photos.length - 1 ? 0 : carouselIndex + 1);
    }, autoplayIntervalMs);
    return () => clearTimeout(timer);
  }, [isAutoPlay, isHovered, photos.length, activePhotoIndex, carouselIndex, autoplayIntervalMs, isLoadingNextPhoto, carouselLoadError, isCurrentCarouselPhotoLoaded, navigateToCarouselPhoto]);
  ...
  const handlePrevCarousel = () => {
    if (photos.length === 0) return;
    void navigateToCarouselPhoto(carouselIndex === 0 ? photos.length - 1 : carouselIndex - 1);
  };
