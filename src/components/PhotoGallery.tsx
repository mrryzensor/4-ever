...
import { useDriveFolderPhotos } from './DriveFolderPhotos.tsx';

type CarouselPhoto = GalleryPhoto & {
  driveOpenUrl?: string;
  driveFileId?: string;
...
export const PhotoGallery: React.FC<PhotoGalleryProps> = ({
  ...
  const getCommentsForPhoto = (photo?: CarouselPhoto | null) => photo
    ? photo.driveFileId
      ? drivePhotoCommentsById[photo.driveFileId] || []
      : photoCommentsMap[photo.id] || []
    : [];
    ...
    const fetchComments = async () => {
      try {
        ...
        if (activePhoto.driveFileId) {
          if (!driveGallery.folderId || !activePhoto.driveInteractionToken) {
            setComments([]);
            return;
          }
          const query = new URLSearchParams({
            weddingId: String(weddingId),
            signature: activePhoto.driveInteractionToken,
          });
          const response = await fetch(`/api/drive-folders/${encodeURIComponent(driveGallery.folderId)}/photos/${encodeURIComponent(activePhoto.driveFileId)}/interactions?${query}`);
          if (!response.ok) throw new Error('No se pudieron cargar las interacciones de Drive.');
          const data = await response.json() as { likesCount?: number; comments?: PhotoComment[] };
          if (cancelled) return;
          ...
          const response = await fetch(`/api/gallery/${activePhoto.id}/comments?weddingId=${weddingId}`);
  ...
  const handleLike = async (photo: CarouselPhoto, e?: React.MouseEvent) => {
    ...
    if (photo.driveFileId) {
      if (likedDrivePhotoIds.includes(photo.driveFileId)) return;
      if (!driveGallery.folderId || !photo.driveInteractionToken) return;
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ ...payload, ...(activePhoto.driveInteractionToken ? { signature: activePhoto.driveInteractionToken } : {}) }),
      });
                      ...
                      <div className="absolute inset-0 z-20 bg-gradient-to-t from-black/95 via-black/35 to-transparent flex flex-col justify-between p-4 sm:p-6 text-white pointer-events-none">
                        ...
                        <div className="space-y-2 pointer-events-auto max-w-2xl">
                          ...
                          <p className="text-xs text-amber-200/90 font-serif italic drop-shadow-sm">
                            {currentCarouselPhoto.driveOpenUrl ? 'Fotos compartidas' : currentCarouselPhoto.authorName ? `Fotografía: ${currentCarouselPhoto.authorName}` : 'Recuerdos de los novios'}
                          </p>
                        </div>
                      </div>
                    </motion.div>
