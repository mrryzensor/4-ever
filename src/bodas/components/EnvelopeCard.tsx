    ...
    <div className="w-full relative transition-colors duration-500" style={{ backgroundColor: theme.bgHex }}>
        ...
        <motion.div style={{ opacity: heroContentOpacity, y: heroContentY, filter: heroContentBlur }} className="relative z-10 w-full h-full flex flex-col justify-between items-center will-change-[opacity,filter,transform] pt-4 sm:pt-10 pb-14 sm:pb-16">
          <div className={`mx-auto my-auto flex w-full max-w-5xl flex-col items-center justify-center px-3 text-center text-white ${heroIsSparse ? 'gap-3 sm:gap-5' : 'gap-1 sm:gap-2'}`}>
            ...
            {heroGuestPillEnabled && guest && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.28 }} className="mt-1 inline-flex max-w-full flex-wrap items-center justify-center gap-x-2 gap-y-1 rounded-full border bg-black/30 px-4 py-2 text-xs text-white shadow-sm backdrop-blur-md sm:text-sm" style={{ borderColor: `${theme.accentColorHex}CC` }}>
            ...
            {heroCountdownEnabled && (
              <div className="mt-2 w-full px-1 sm:mt-3">
                <AnimatedCountdown settings={settings} guest={guest} cardStyle={settings.cardStyle} customStyle={settings.countdownStyle} customTitle={settings.countdownTitle} showGuestsBadge={false} compact />
              </div>
            )}
      ...
      >
        ...
        <div className="relative z-10 w-full max-w-7xl 2xl:max-w-[1600px] mx-auto text-center overflow-visible">
          ...
          {settings.showCountdown !== false && !settings.heroShowCountdown && (
