...
const renderHeroEmblem = (heroIconStyle?: string, cardStyle: string = 'classic-gold', sparse = false, themeAccent?: string, emblemColor?: string, glowIntensity?: number, sparkleIntensity?: number, emblemScale?: number) => {
  ...
  return <HeroEmblem cardStyle={effectiveIcon} accentColor={accentColor} sparse={sparse} color={emblemColor} glowIntensity={glowIntensity} sparkleIntensity={sparkleIntensity} scale={emblemScale} quinceanera />;
};
...
export const EnvelopeCard: React.FC<EnvelopeCardProps> = ({
  ...
  const googleCalendarUrl = useMemo(() => {
    ...
    return `https://calendar.google.com/calendar/render?action=TEMPLATE&text=Mis+XV+A%C3%B1os+-+${encodeURIComponent(coupleNamesSafe)}&dates=${startStr}/${endStr}&details=Celebraci%C3%B3n+de+mis+XV+A%C3%B1os.&location=${encodeURIComponent(receptionLocationName)}`;
